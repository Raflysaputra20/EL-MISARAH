...
try {
    $stmtBulanIni = $conn->prepare("
        SELECT p.id, p.jumlah, p.status, p.metode 
        FROM pembayaran p
...
$statusBulanIni  = $pembayaranBulanIni ? $pembayaranBulanIni['status'] : 'belum_bayar';
        ...
        <div class="row g-4">
            <div class="col-md-7">
                <div class="glass-card">
                    <span style="font-size:12px; color:var(--gray); font-weight:700; letter-spacing:1px; text-transform:uppercase;">Tagihan Bulan Ini</span>
                    ...
                    <div class="status-pill <?= $statusBulanIni === 'valid' ? 'pill-lunas' : ($statusBulanIni === 'menunggu_verifikasi' ? 'pill-verif' : 'pill-belum') ?>">
                        ...
                        <?= $statusBulanIni === 'valid' ? 'Sudah Lunas' : ($statusBulanIni === 'menunggu_verifikasi' ? 'Verifikasi Admin' : 'Belum Bayar') ?>
                    </div>
                    <?php if ($statusBulanIni !== 'valid' && $statusBulanIni !== 'menunggu_verifikasi'): ?>
            ...
            <table class="table table-hover mb-0" style="font-size:13.5px;">
                ...
                <?php foreach ($riwayatBayar as $r): ?>
                    <tr>
                        ...
                        <td><span class="badge <?= $r['status']==='valid'?'bg-success-subtle text-success':'bg-warning-subtle text-warning' ?> rounded-pill px-3 py-2"><?= ucfirst($r['status']) ?></span></td>
...
<script>
    ...
    function kirimBayarWA() {
        if (!document.getElementById('buktiI').files[0]) { alert("Upload bukti dulu!"); return; }
        const m = encodeURIComponent(`Halo Admin, saya *<?= addslashes($namaUser) ?>* (Kamar <?= $nomorKamar ?>) konfirmasi bayar tagihan *Rp <?= number_format($tagihanBulanIni,0,',','.') ?>*. Bukti sudah di-upload.`);
        window.open(`https://wa.me/<?= $adminWA ?>?text=${m}`, '_blank'); location.reload();
    }
